...
: ${PROD_ID_NO_REVS=213}
: ${SKIP_CB_TESTS=false}
: ${NAMESPACE=loan-origination-system}
...
function testCompositeCreated() {
    ...
    if ! assertCurl 200 "curl $AUTH -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS -s"
    then
        echo -n "FAIL"
        return 1
    fi
...
function setupTestdata() {
  ...
  body="{\"productId\":$PROD_ID_NO_RATS"
  body+=\
',"name":"product name A","description": "product A", "reviews":[
  {"reviewId":1,"author":"author 1","subject":"subject 1","content":"content 1"},
  {"reviewId":2,"author":"author 2","subject":"subject 2","content":"content 2"},
  {"reviewId":3,"author":"author 3","subject":"subject 3","content":"content 3"}
]}'
  recreateComposite "$PROD_ID_NO_RATS" "$body"

  body="{\"productId\":$PROD_ID_NO_REVS"
  body+=\
',"name":"product name B","description": "product B", "ratings":[
  ...
  body="{\"productId\":$PROD_ID_RATS_REVS"
  body+=\
',"name":"product name C","description": "product C", "ratings":[
      {"ratingId":1,"author":"author 1","rate":1,"content":"content 1"},
      {"ratingId":2,"author":"author 2","rate":2,"content":"content 2"},
      {"ratingId":3,"author":"author 3","rate":3,"content":"content 3"}
  ], "reviews":[
  ...
  recreateComposite "$PROD_ID_RATS_REVS" "$body"
...
function testCircuitBreaker() {
    ...
    for ((n=0; n<3; n++))
    do
        assertCurl 500 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS?delay=3 $AUTH -s"
    ...
    assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS?delay=3 $AUTH -s"
    assertEqual "Fallback product$PROD_ID_RATS_REVS" "$(echo "$RESPONSE" | jq -r .name)"
    ...
    assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS $AUTH -s"
    assertEqual "Fallback product$PROD_ID_RATS_REVS" "$(echo "$RESPONSE" | jq -r .name)"
...
# Verify that a normal request works, expect three ratings and three reviews
assertCurl 200 "curl $AUTH -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS -s"
assertEqual $PROD_ID_RATS_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".ratings | length")
assertEqual 3 $(echo $RESPONSE | jq ".reviews | length")
...
# Verify that no ratings are returned for productId $PROD_ID_NO_RATS
assertCurl 200 "curl $AUTH -k https://$HOST:$PORT/product-composite/$PROD_ID_NO_RATS -s"
assertEqual $PROD_ID_NO_RATS $(echo $RESPONSE | jq .productId)
...
assertCurl 401 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS -s"
...
assertCurl 200 "curl $READER_AUTH -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS -s"
assertCurl 403 "curl -X DELETE $READER_AUTH -k https://$HOST:$PORT/product-composite/$PROD_ID_RATS_REVS -s"
