...
validate_environment() {
    ...
    if [[ ! -f "$PROMPT_FILE" ]]; then
        log "${RED}Error: Prompt file (prompt.md or CLAUDE.md) not found in scripts/ralph/${NC}"
        exit 1
    fi
    ...
    # Check for plan file
    if [[ -f "$PROJECT_ROOT/specs/003-ai-chat-box/tasks.md" ]]; then
        log "${GREEN} + ${NC}Plan file: tasks.md"
    elif [[ -f "$PROJECT_ROOT/specs/003-ai-chat-box/prd.json" ]]; then
        log "${GREEN} + ${NC}Plan file: prd.json"
    else
        log "${YELLOW}Warning: No plan file found (tasks.md or prd.json)${NC}"
    fi
}
...
run_ralph() {
    ...
    # Calculate estimated cost using bc for float math
    MAX_COST=$(echo "$MAX_ITERATIONS * $ESTIMATED_COST_PER_ITERATION" | bc)
    log "Estimated max cost: \$${MAX_COST}"
    ...
    for ((i=1; i<=MAX_ITERATIONS; i++)); do
        ...
        if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
            ...
            ITERATION_COST=$(echo "$i * $ESTIMATED_COST_PER_ITERATION" | bc)
            log "Estimated cost: \$${ITERATION_COST}"
            exit 0
        fi

        # Check for explicit failure signals
        if echo "$OUTPUT" | grep -qiE "BLOCKED|STUCK|CANNOT PROCEED"; then
    ...
    log "Total time: ${TOTAL_TIME}s"
    FINAL_COST=$(echo "$MAX_ITERATIONS * $ESTIMATED_COST_PER_ITERATION" | bc)
    log "Estimated cost: \$${FINAL_COST}"
    log ""
    log "Next steps:"
